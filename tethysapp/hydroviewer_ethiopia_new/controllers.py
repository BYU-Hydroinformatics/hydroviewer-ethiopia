...
def ecmwf_get_time_series(request):
    ...
    try:
        ...
        layout = go.Layout(
            title="Forecast<br><sub>{0} ({1}): {2}<br>Total upstream drainage area: {3} km<sup>2</sup></sub>".format(
...
def get_historic_data(request):
    ...
    try:
        ...
        layout = go.Layout(
            title="Historical Streamflow<br><sub>{0} ({1}): {2}<br>Total upstream drainage area: {3} km<sup>2</sup></sub>".format(
                watershed, subbasin, comid, tot_drain_area),
            xaxis=dict(
                title='Date',
            ),
